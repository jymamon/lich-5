...
class Map
  ...
  attr_reader :id
  attr_accessor :title, :description, :paths, :uid, :location, :climate, :terrain, :wayto, :timeto, :image, :image_coords, :tags, :check_location, :unique_loot, :room_objects
  ...
  def self.last_seen_objects=(val)
    @@last_seen_objects = val
  end

  def self.last_seen_objects
    @@last_seen_objects
  end
      ...
      @@current_room_mutex.synchronize {
        if XMLData.room_count == @@current_room_count
          ...
          begin
            ...
            # rubocop:enable Lint/UselessTimes Used to allow 'redo'.
      ...
      check_peer_tag = proc { |r|
        if (peer_tag = r.tags.find { |tag| tag =~ /^(set desc on; )?peer [a-z]+ =~ \/.+\/$/ })
          ...
          if result =~ /^You peer/
            ...
            good = true if peer_results.any? { |line| line =~ /#{peer_requirement}/ }
          end
          script.want_downstream = save_want_downstream
        else
          good = true
