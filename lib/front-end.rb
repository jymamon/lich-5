...
module Frontend
  ...
  def self.create_session_file(name, host, port)
    ...
    @session_file = File.join(@tmp_session_dir, sprintf('%s.session', name.downcase.capitalize))
    session_descriptor = { name: name, host: host, port: port }.to_json
    puts sprintf("writing session descriptor to %s\n%s", @session_file, session_descriptor)
    File.open(@session_file, 'w') { |fd|
