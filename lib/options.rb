...
class Parser
  ...
  def self.parse(options)
    ...
    Thanks to all those who\'ve reported bugs and helped me track down problems on both Windows and Linux.
      ...
      opts.on('--gemstone',
              'Login to GemStone IV. Defaults to the Prime instance. Only needed if the same CHARACTER exists for mutliple games.') do
        args.gemstone = true
      end

      opts.on('--fallen',
              'Login to the Fallen instance of DragonRealms. Only needed if the same CHARACTER exists for mutliple instances.') do
        args.fallen = true
      end

      opts.on('--platinum',
              'Login to the Platinum instance of the game. Defaults to GemStone IV. Only needed if the same CHARACTER exists for instances.') do
        args.platinum = true
      end

      opts.on('--shattered',
              'Login to the Shattered instance of GemStone IV. Only needed if the same CHARACTER exists for instances.') do
        args.shattered = true
      end

      opts.on('--test',
              'Login to the Test instance of the game. Defaults to GemStone IV. Only needed if the same CHARACTER exists for instances.') do
        args.test = true
      end
    ...
    if sal_file_index
      if File.exist?(options[sal_file_index])
        ...
        options.sal = ::Regexp.last_match(1)
      ...
      if !File.exist?(options.sal) && defined?(Wine)
        ...
        options.sal = "#{Wine::PREFIX}/drive_c/#{argv_options[:sal][3..].split('\\').join('/')}"
      end
    end
